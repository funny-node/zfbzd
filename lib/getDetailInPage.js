...
function getDetailInPage(options) {
  ...
  items.forEach(item => {
    let date = item.querySelector('.time p').innerHTML.trim()
    ...
    if (date === '今天') date = changeTimestampToFormate(+new Date)
    else if (date === '昨天') date = changeTimestampToFormate(+new date - 3600*24*1000)
  ...
  */ 
   ...
   */
  function changeTimestampToFormate(timestamp) {
